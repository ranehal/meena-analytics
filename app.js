(() => {
  ...
  const shardName = id => `history_${(id % 256).toString(16).padStart(2, "0")}.json`;
  const historyCache = new Map();

  function expandSegments(segments) {
    const points = [];
    for (const segment of segments || []) {
      const from = new Date(segment.from + "T00:00:00");
      const to = new Date(segment.to + "T00:00:00");
      if (isNaN(from.getTime())) continue;
      const end = isNaN(to.getTime()) ? from : to;
      for (let day = new Date(from); day <= end; day.setDate(day.getDate() + 1)) {
        points.push({ date: new Date(day), price: segment.price });
      }
    }
  ...
  async function loadHistoryShard(shard) {
    if (historyCache.has(shard)) return historyCache.get(shard);
    const res = await fetch(shard);
    if (!res.ok) throw new Error(`Failed to load ${shard}`);
    const data = await res.json();
    const byId = new Map();
    Object.entries(data.products || {}).forEach(([id, segments]) => {
      const points = expandSegments(segments);
      if (points.length) byId.set(Number(id), points);
    });
    historyCache.set(shard, byId);
    return byId;
  }

  async function hydrateHistory(product) {
    const byId = await loadHistoryShard(product.shard);
    const points = byId.get(product.id);
    if (points) {
      product.history = points;
      product.average = averageOf(points.map(point => point.price));
      product.lowest = Math.min(...points.map(point => point.price));
      product.trend = product.price - points[Math.max(0, points.length - 8)].price;
      product.historyLoaded = true;
    }
    return !!points;
  }

  const lifetimeIsLowest = product => product.historyLoaded && metrics(product, product.history.length).isLowest;
  const windowIsLowest = product => product.historyLoaded && metrics(product).isLowest;
  ...
  function dealClass(product) {
    ...
    if (lifetimeIsLowest(product)) return "alltime";
    if (windowIsLowest(product)) return "lowest";
  ...
  function dealLabel(product) {
    const m = metrics(product);
    if (lifetimeIsLowest(product)) return "All-time low";
    if (windowIsLowest(product)) return `Lowest in ${state.window} days`;
  ...
  function filteredProducts() {
    ...
    return products.filter(product => {
      ...
      const matchesDeal = state.deal === "all"
        ...
        || (state.deal === "alltime" && lifetimeIsLowest(product))
        || (state.deal === "lowest" && windowIsLowest(product));
  ...
  function sortProducts(items) {
    return items.sort((a, b) => {
      ...
      switch (state.sort) {
        ...
        default: {
          const scoreA = a.discount * 1.2 - ma.vsAvgPct + (windowIsLowest(a) ? 18 : 0) + (lifetimeIsLowest(a) ? 25 : 0) - (a.inStock ? 0 : 100);
          const scoreB = b.discount * 1.2 - mb.vsAvgPct + (windowIsLowest(b) ? 18 : 0) + (lifetimeIsLowest(b) ? 25 : 0) - (b.inStock ? 0 : 100);
      ...
      if (res.ok) {
        ...
        products = data.products.map((p, index) => {
          ...
          return {
            ...
            background, accent, history, shard, historyLoaded: false,
  ...
  async function prefetchAllHistory() {
    const shards = products.map(p => p.shard);
    const unique = [...new Set(shards)];
    let cursor = 0;
    async function worker() {
      while (cursor < unique.length) {
        const shard = unique[cursor++];
        try {
          const byId = await loadHistoryShard(shard);
          for (const product of products) {
            if (product.shard !== shard || product.historyLoaded) continue;
            const points = byId.get(product.id);
            if (points) {
              product.history = points;
              product.average = averageOf(points.map(point => point.price));
